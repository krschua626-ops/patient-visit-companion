...
export function VisitDashboardPage() {
  ...
  const { visit, timing, study, outstanding_tasks } = data
  ...
  return (
    <>
      <PageHeader
        title={isPast ? 'Most recent visit' : 'Your visit'}
        subtitle={`${visit.name} · ${timing.label}`}
      />
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4 pb-24">
        <div>
          ...
          <Link to={`/visit/${visit.id}/briefing`} className="block group">
